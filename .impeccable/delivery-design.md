...
Logo upload is a square clickable tile to the left of the sheet-name field, sharing one identity row with a 12px gap. The tile is 64px square on desktop and 72px at the compact/coarse breakpoint; the right-hand label and input align to the same outer height. The separate format-help paragraph is removed; constraints remain in the tile title and file-input accessible label. Optional Remove appears below only when a Logo exists. Loading and invalid-image feedback remain visible within this compact identity group. Tag input and Add use a flexible-plus-auto grid with matching top and bottom edges. Wrapped color wells start on the same content rail as the palette.

The Skill tab places the library heading, search and a flat checklist before upload and editable selections. Results scroll within a desktop maximum height (228px), increasing at widths up to 600px (252px). Full-width rows have a minimum height (76px), padding (12px 8px), and a 16px native checkbox on the left. Row gaps reduce from 10px to 8px on narrow screens; commands move below names. Descriptions and source labels wrap. Search follows the existing field heights with 34px left padding, keeping its icon clear of the text.

Member management is a flat roster beneath the customer/target row. Rows pair a circular initial (28px) with name, native role selector (188px desktop) and removal. Rows have a minimum height of 48px and 6px vertical padding. At widths up to 440px, role/removal controls move below the name, aligned to its 36px text inset. Coarse-pointer selects are at least 44px high with 16px text. The search opens a native top-layer popover (maximum 264px high, clamped above/below the input to available viewport space); full-width 48px checklist rows permit repeated selection without dismissal.
...
Save is gated by required name/customer, integer target 1–100000, a nonempty List for new sheets or changed lists, active file reading, List errors and invalid/duplicate same-sheet commands. Rejected optional Logo/Skill files show alerts but their error strings are not themselves Save gates.
...
Rows are full-width labels with native checkboxes on the left. Accessible names include name, command and source/version; descriptions connect through `aria-describedby`. Unselected rows show an empty checkbox, while selected rows use a checked Accent control and Accent Soft background. The former right-side plus/选择 action is removed. Hover uses Subtle only for enabled, unselected rows; keyboard focus outlines the whole row with an inset 2px Accent ring. Loading disables all checkboxes; reaching 12 selections disables only unselected rows, preserving removal. Selection stays a draft until Save.

### Sheet Members / Roles

Keep member configuration inside 基础信息. Search matches names, accounts and directory details. Click, typing or ArrowDown opens a native popover; outside click, Escape, focus outside, switching tabs and closing the editor dismiss it. Left-side native checkboxes toggle selection, and selected people remain available for deselection. Gray hover/selected backgrounds and dark-neutral checkbox/focus are deliberate exceptions to the orange selected-Skill pattern. The result count is announced; empty and 50-person capacity states remain readable. New additions default to Reviewer-Forge.

Owner, Reviewer-Forge and Reviewer-Outsourcing are the only sheet-local roles, not changes to the account's global role. Legacy reviewer/member map to reviewer-forge and outsourcing to reviewer-outsourcing. The creator starts as sheet Owner. The local demo permits management by a sheet Owner, the owning project's Owner, or Lead; other viewers receive a read-only roster. These checks are prototype behavior, not server authorization or persisted invitations.

At least one Owner must remain: disable removal, checkbox deselection and role changes for the final Owner until another Owner is assigned. Changes remain draft until Save, which rechecks the actor, current permissions and roster baseline. Native role options require explicit `label` attributes because the template compiler wraps interpolated text. Status messages are announced without a layout-expanding paragraph; disabled Owner controls explain the constraint in their title. New-sheet creation queues one local inbox reminder for each non-creator member, linking to the correct sheet after access checks. Feishu is explicitly not connected, not presented as sent.
