...
test('queue markup has six stable columns, semantic table, labels and no alarm design',()=>{
 ...
 assert.match(css,/\.pq-filter\{[^}]*box-sizing:border-box[^}]*height:36px[^}]*min-height:36px[^}]*max-height:36px[^}]*padding-left:0/);
 assert.match(css,/\.pq-filter select\{[^}]*box-sizing:border-box[^}]*height:34px!important[^}]*min-height:34px!important[^}]*max-height:34px[^}]*padding:4px 22px 4px 10px/);
 assert.match(css,/\.pq-search input\{[^}]*box-sizing:border-box[^}]*height:36px!important[^}]*min-height:36px!important[^}]*max-height:36px/);
