...
export const overviewSummaryCopy = [
  ["this.deliveryData().forEach(c => c.sheets.forEach(x => flat.push(Object.assign({ customer: c.customer }, x))));\n      over = {", "this.deliveryData().forEach(c => c.sheets.forEach(x => flat.push(Object.assign({ customer: c.customer }, x))));\n      const overviewDeliveryDate = ['week', 'fortnight', 'later'].includes(st.overviewDeliveryDate) ? st.overviewDeliveryDate : 'all';\n      const overviewDeliveryRows = flat.map((d, index) => ({ d, index, days: Number.isFinite(d.deliveryInDays) ? d.deliveryInDays : 7 + index * 3 })).filter(({ days }) => overviewDeliveryDate === 'all' || (overviewDeliveryDate === 'week' && days <= 7) || (overviewDeliveryDate === 'fortnight' && days > 7 && days <= 14) || (overviewDeliveryDate === 'later' && days >= 15));\n      over = {"],
  ["title: '交付进度', count: flat.length + ' 张数据单'", "title: '相关交付进度', count: overviewDeliveryRows.length + ' 张数据单', hasRows: flat.length > 0"],
  ["supplierPerformance: this.supplierPerformanceValues(),\n        groups:", "supplierPerformance: this.supplierPerformanceValues(),\n        deliveryDateFilter: overviewDeliveryDate,\n        setDeliveryDateFilter: event => this.setState({ overviewDeliveryDate: event.target.value }),\n        groups:"],
  ...
  ["empty: flat.length === 0, emptyLabel: '还没有交付数据单'", "empty: overviewDeliveryRows.length === 0, emptyLabel: overviewDeliveryDate === 'all' ? '还没有交付数据单' : '没有符合日期条件的交付数据单'"],
];
export function refineOverviewSummary(t) {
  ...
  const columns='<sc-if value="{{ g.hasRows }}"><div class="pm-overview-delivery-columns"><span>数据单</span><span>项目负责人</span><label class="pm-overview-delivery-date-filter"><select aria-label="筛选交付日期" value="{{ over.deliveryDateFilter }}" sc-camel-on-change="{{ over.setDeliveryDateFilter }}"><option value="all">交付日期</option><option value="week">7 天内</option><option value="fortnight">8–14 天</option><option value="later">15 天后</option></select></label><span>交付状态</span></div></sc-if>\n';
