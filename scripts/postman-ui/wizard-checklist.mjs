...
export const wizardChecklistCopy = [
 ...
 ['stats, hasEntries: !!editor.entries.length, noRows: !rows.length, rows, onlyExceptions: !!editor.onlyExceptions,\n      onExceptions: event => patch({ onlyExceptions: event.target.checked, entryPage: 1 }),', "stats, hasEntries: !!editor.entries.length, noRows: !rows.length, rows, candidateCount:filtered.length, selectedCount:editor.entries.length, selectionSummary:'已选 ' + editor.entries.length + ' / 共 ' + filtered.length + ' 条', selectAllDisabled:!filtered.length || editor.entries.length===filtered.length, clearSelectionDisabled:!editor.entries.length, selectAll:()=>this.pmSelectAllWizardEntries(id), clearSelection:()=>this.pmClearWizardEntries(id), // pm-preview-all-rows"],
 ...
 ["      if (excluded.has(entry.itemId)) return;", "      // Keep every candidate in the pool; selection is applied after deduplication."],
 ["      const previous = editor.entries.find(row => row.itemId === entry.itemId);", "      const previous = editor.entries.find(row => row.itemId === entry.itemId) || (editor.pmWizardPool || []).find(row => row.itemId === entry.itemId);"],
 ["    this.patchDeliveryEditor({ importMode: 'production', productionTaskIds: requested, archive: null, entries,\n      listText: entries.map(entry => entry.source).join('\\n'), listChanged: true, entryPage: 1, error: '',", "    const targetCount = Math.max(0, Math.min(Number(editor.target) || 0, entries.length));\n    const selectionTouched = editor.pmWizardSelectionTouched || !!editor.key;\n    const selectedEntries = selectionTouched ? entries.filter(entry => !excluded.has(entry.itemId)) : entries.slice(0, targetCount);\n    const selectedKeys = new Set(selectedEntries.map(entry => entry.key));\n    this.patchDeliveryEditor({ pmWizardPool: entries.map(entry => ({ ...entry })), importMode: 'production', productionTaskIds: requested, archive: null, entries: selectedEntries,\n      productionExcluded: entries.filter(entry => !selectedKeys.has(entry.key)).map(entry => entry.itemId),\n      listText: selectedEntries.map(entry => entry.source).join('\\n'), listChanged: true, entryPage: 1, error: '',"]
];
export function installWizardChecklist(t) {
 ...
 t=t.replace('<span role="columnheader">匹配状态</span>', '');
 ...
 t=t.replace('<span role="columnheader">Tag</span>', '');
 ...
 const previewStart=t.indexOf('<div class="forge-wizard-table" role="table" aria-label="同步条目预览">');
 const previewEnd=t.indexOf('</section>',previewStart);
 if(previewStart<0||previewEnd<previewStart)throw Error('Wizard preview table boundary changed');
 const preview=t.slice(previewStart,previewEnd)
  .replace(/<div role="cell"><span class="forge-wizard-status"[^>]*>[\s\S]*?<\/span><\/div>/,'')
  .replace(/<div role="cell"><select[^>]*sc-camel-on-change="\{\{ entry\.onTag \}\}"[^>]*>[\s\S]*?<\/select><\/div>/,'');
 t=t.slice(0,previewStart)+preview+t.slice(previewEnd);
 t=t.replace(/<label class="forge-wizard-checkbox"><input[^>]*sc-camel-on-change="\{\{ deliveryEditor.wizard.onExceptions \}\}"[^>]*\/>只看异常<\/label>/,'');
 const previewHeadingAnchor='<div class="forge-delivery-section-heading pm-wizard-preview-heading"><h3>条目预览</h3><span class="forge-wizard-preview-summary" role="status">共 {{ deliveryEditor.wizard.stats.recognized }} 条</span></div>';
 const previewHeading='<div class="forge-delivery-section-heading pm-wizard-preview-heading"><div class="pm-wizard-preview-title"><h3>条目预览</h3><span class="forge-wizard-preview-summary" role="status">{{ deliveryEditor.wizard.selectionSummary }}</span></div><div class="pm-wizard-selection-actions" aria-label="Item 批量选择"><button type="button" class="forge-delivery-text-button pm-wizard-select-all" disabled="{{ deliveryEditor.wizard.selectAllDisabled }}" sc-camel-on-click="{{ deliveryEditor.wizard.selectAll }}">全选</button><button type="button" class="forge-delivery-text-button pm-wizard-clear-selection" disabled="{{ deliveryEditor.wizard.clearSelectionDisabled }}" sc-camel-on-click="{{ deliveryEditor.wizard.clearSelection }}">取消全选</button></div></div>';
 if(!t.includes(previewHeadingAnchor))throw Error('Wizard preview heading anchor changed');
 t=t.replace(previewHeadingAnchor,previewHeading);
 t=t.replace('<p class="forge-wizard-empty">没有可展示的条目，请重新同步生产任务或关闭异常筛选。</p>', '<div class="pm-wizard-empty-row" role="row"><div role="cell" aria-colspan="2" class="forge-wizard-empty">选择 Pipeline × 数据集的生产任务，或上传 ZIP 后，条目会显示在这里。</div></div>');
 ...
 t=t.replace('<strong>{{ task.name }}<sc-if value="{{ task.demo }}"><small>示例</small></sc-if></strong><span title="{{ task.pipeline }}">{{ task.pipeline }}</span><span title="{{ task.dataset }}">数据集 · {{ task.dataset }}</span>', '<strong><span>{{ task.pipeline }} × {{ task.dataset }}</span><sc-if value="{{ task.demo }}"><small>示例</small></sc-if></strong><span class="pm-wizard-task-context"><span title="{{ task.name }}">{{ task.name }}</span><span aria-hidden="true">·</span><span title="{{ task.id }}">Run ID {{ task.id }}</span></span>');
