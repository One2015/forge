  ...
  pmSheetReferenceImages(images) {
    return (Array.isArray(images) ? images : []).filter(image => image && !image.loading && typeof image.url === 'string' && /^(?:https?:\/\/|blob:|\/(?!\/)|data:image\/(?:png|jpeg|webp|gif);base64,)/i.test(image.url))
