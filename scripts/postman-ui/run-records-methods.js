  ...
  runRecordsValues() {
    ...
    const clear=()=>patch({runsQuery:'',runsFilter:'全部',runsMine:false,runsOwner:''});
    ...
    const matches=all.rows.filter(r=>(filter==='all'||r.state===filter)&&(!owner||r.owner===owner)&&(!query||[r.strategy,r.name,r.pipe,r.dsName,r.id,r.owner].join(' ').toLowerCase().includes(query))).sort((a,b)=>b.started-a.started);
    ...
    const rows=matches.slice((page-1)*size,page*size).map(r=>{
      ...
      const hasReviewable=counts.review>0, allFailed=r.n>0&&counts.failed===r.n;
      const action=hasReviewable?'review':allFailed||r.state==='failed'?'cause':'detail';
      ...
      return {...r,isRunning:r.state==='running',isComplete:r.state==='completed',statusLabel:labels[r.state],progress,durationLabel:duration,durationHint:r.duration==null?'当前记录未提供执行耗时':(r.durationSynthetic?'Mock 执行时长快照 · ':'')+(r.state==='running'?'已运行时长':'执行耗时'),costLabel:all.money(r.costNumber),when:this.ago(Number(Math.max(0,(now-r.started)/3600000).toFixed(1))),timeFull:new Date(r.started).toLocaleString('zh-CN',{hour12:false}),
        ...
        actionTone:action,actionLabel:({cause:'查看原因',review:'人工审核',detail:detailLabel})[action],action:action==='review'?review:action==='cause'?failed:detail};
    });
    ...
    const filterCount=Number(filter!=='all')+Number(!!owner);
    ...
    const statusFilter=filterControls[0],ownerFilter=filterControls[1];
    ...
    return {rows,kpis,subtitle:raw.length+' 次运行',count:matches.length+' / '+raw.length,hasAny:!!raw.length,empty:!matches.length,emptyTitle:raw.length?'没有匹配的运行':'还没有运行记录',emptyHint:raw.length?'试试减少筛选条件，或换个关键词。':'从 Pipeline 发起运行后，记录会显示在这里。',query:st.runsQuery||'',onQuery:e=>patch({runsQuery:e.target.value}),filter,setFilter:e=>patch({runsFilter:e.target.value==='all'?'全部':labels[e.target.value]||'全部'}),owner,filterCount,hasFilterSelections:filterCount>0,filterControls,statusFilter,ownerFilter,activeFilter,filterMenuOpen:!!activeFilter,closeFilterMenu,filterMenuKey,
      ...
      hasFilters:!!query||filter!=='all'||!!owner,clear,metricLabel:'',hasMetric:false,
