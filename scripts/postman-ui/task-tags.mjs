...
export const taskTagCopy = [
 ...
 ["taskTags: this.deliveryEntryTagPicker(editor, entry)", "taskTags: this.pmTaskTagPicker(editor,entry)"],
 ...
 ['      defaultTagId: editor.defaultTagId || \'\', defaultTagName:',"      taskTags:this.pmTaskTagPicker(editor), taskTagNames:this.pmTaskTagSummaryNames(editor), defaultTagId: editor.defaultTagId || '', defaultTagName:"],
 ...
 ["tagEmpty: tagIds.size === 0", "tagEmpty: new Set(editor.entries.flatMap(entry=>this.pmTaskTagIds(editor,entry)).concat(this.pmTaskTagIds(editor))).size === 0"],
...
function picker(path){return `<details class="pm-task-tag-picker" style="--pm-task-tag-width:{{ ${path}.width }}"><summary aria-label="{{ ${path}.label }}"><span class="pm-task-tag-chips"><sc-for list="{{ ${path}.selected }}" as="chosen"><span class="pm-task-tag-chip" style="--task-tag-bg:{{ chosen.color }};--task-tag-fg:{{ chosen.fg }}">{{ chosen.name }}</span></sc-for><sc-if value="{{ ${path}.empty }}"><span>未设置</span></sc-if></span>${phosphorIcon('caret-down',12,{className:'pm-task-tag-caret'})}</summary><div class="pm-task-tag-options" role="radiogroup" aria-label="{{ ${path}.label }}"><sc-for list="{{ ${path}.options }}" as="choice"><label><input type="radio" name="{{ ${path}.inputName }}" checked="{{ choice.checked }}" disabled="{{ ${path}.readonly }}" sc-camel-on-change="{{ choice.toggle }}" /><span class="pm-task-tag-chip" style="--task-tag-bg:{{ choice.color }};--task-tag-fg:{{ choice.fg }}">{{ choice.name }}</span></label></sc-for><sc-if value="{{ ${path}.noOptions }}"><span>请先创建 Tag</span></sc-if></div></details>`;}
