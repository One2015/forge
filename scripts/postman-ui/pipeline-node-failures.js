  pmPipelineNodeFailures(key) {
    ...
    for(const [itemId,byRun] of Object.entries(this.props.itemExecution || {}))for(const [runId,execution] of Object.entries(byRun || {})){
      const run=runs.find(run=>run.id===runId),pipeline=execution.pipeline?.name || run?.pipe || run?.pipeline || run?.name?.split(' · ').at(-1)?.replace(/\s+v\d+$/,'');
      const context={pipeline,version:execution.pipeline?.version || run?.ver || run?.pipelineVersion,runId,itemId};
      for(const [node,record] of Object.entries(execution.nodes || {}))add({...record,node},context);
    }
