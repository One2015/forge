  ...
  modelDemoInput() {
    ...
    const specs = [
      { id: 'production-02', modelId: 'claude-sonnet', providerId: 'yunqiao', current: 11910, baseline: 3420, total: 18600, totalBaseline: 7750, tps: 42, tpsDelta: -8, calls: 1800, failures: 126, cost: 78.4, quality: 93, qualityBase: 94, error: 'timeout', status: 'severe', action: '联系供应商', impact: { tasks: 2, project: 'Vision2Web', ddl: '距 DDL 1 天', failedCalls: 126 }, cause: '更可能是供应商排队或线路拥堵。', explanation: 'TTFT 明显升高，但生成速度基本稳定；官方直连未出现相同异常。', errors: '504 超时增加 98 次 · 429 限流 28 次', qualityKind: 'normal', started: '今天 12:35' },
      { id: 'production-01', modelId: 'gpt-41', providerId: 'yunqiao', current: 5580, baseline: 3100, total: 9100, totalBaseline: 6200, tps: 46, tpsDelta: 1, calls: 950, failures: 11, cost: 42.6, quality: 92, qualityBase: 93, error: '', status: 'performance', action: '查看详情', impact: { tasks: 1, project: 'Web3D 中国地标', ddl: '距 DDL 4 天', failedCalls: 11 }, cause: '供应商高峰排队增加，尚未达到严重阈值。', explanation: 'TTFT 上升 1.8×，生成速度与错误率仍处于基线范围。', errors: '偶发 504 超时 11 次', qualityKind: 'normal', started: '今天 13:10' },
      { id: 'proxy-03', modelId: 'gpt-4o-mini', providerId: 'qinghe', current: 1760, baseline: 1600, total: 6800, totalBaseline: 6400, tps: 51, tpsDelta: -2, calls: 2200, failures: 9, cost: 18.2, quality: 76.4, qualityBase: 86.8, error: '', status: 'quality', action: '运行对比测试', impact: { tasks: 0, project: 'WebDev Eval', ddl: '距 DDL 3 天', failedCalls: 9 }, cause: '固定测试集表现出现质量漂移。', explanation: '延迟和错误率正常，但 Benchmark 连续三轮低于历史基线。', errors: '错误率稳定，无集中错误码', qualityKind: 'drift', started: '昨天 18:20' },
      { id: 'proxy-07', modelId: 'claude-sonnet', providerId: 'beichen', current: 4080, baseline: 3400, total: 7900, totalBaseline: 7200, tps: 44, tpsDelta: -3, calls: 860, failures: 7, cost: 35.3, quality: 88, qualityBase: 92, error: '', status: 'confirm', action: '立即复测', impact: { tasks: 0, project: '模型一致性测试', ddl: '今日待完成', failedCalls: 7 }, cause: '存在疑似模型不一致信号，需要复测确认。', explanation: 'Fingerprint 发生变化，与官方直连的固定测试结果差异 18%。', errors: '无集中错误码', qualityKind: 'mismatch', fingerprint: 'fp_9c2 → fp_41a', directGap: 18, started: '今天 11:50' },
      { id: 'official-opus', line: '官方直连', modelId: 'claude-opus', providerId: 'anthropic', current: 3020, baseline: 3150, total: 8200, totalBaseline: 8400, tps: 38, tpsDelta: 2, calls: 540, failures: 2, cost: 6, quality: 91, qualityBase: 91, error: '', status: 'billing', action: '去充值', impact: { tasks: 0, project: '新任务可能失败', ddl: '预计 6 小时后耗尽', failedCalls: 2 }, cause: '账户余额即将低于安全阈值。', explanation: '当前调用仍正常，但按近 1 小时消耗速度预计仅可用 6 小时。', errors: '暂无余额导致的失败', qualityKind: 'normal', started: '今天 14:00' },
    ...
    const routes = specs.map(spec => {
      ...
      return {
        ...
        usage: { calls: spec.calls, failures: spec.failures, costUsd: spec.cost, checkedAt: at, windowLabel: '近 1 小时' },
        demoEvidence: { line: spec.line || spec.id, status: spec.status, action: spec.action, cause: spec.cause, explanation: spec.explanation, errors: spec.errors, qualityKind: spec.qualityKind, fingerprint: spec.fingerprint || '', directGap: spec.directGap || 0, totalMs: spec.total, totalBaselineMs: spec.totalBaseline, tokensPerSecond: spec.tps, tokensPerSecondDelta: spec.tpsDelta, impact: spec.impact, abnormalStartedAt: spec.started,
