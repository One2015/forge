...
test('delivery browser table header filters creators and sorts one field at a time',()=>{
 ...
 assert.doesNotMatch(page,/delivery\.sortLabel|>最新</);
 ...
 assert.match(page,/class="pm-delivery-table-head"[\s\S]*>客户<[\s\S]*>数据单名字<[\s\S]*>创建时间<[\s\S]*>创建人<[\s\S]*>状态<[\s\S]*>目标数<[\s\S]*>已关联条数<[\s\S]*>待审核条数<[\s\S]*>交付条数<[\s\S]*>进度<[\s\S]*>操作</);
 assert.equal((page.match(/class="pm-delivery-sort-button"/g)||[]).length,5);
 assert.equal((page.match(/aria-sort="\{\{ delivery\.sort(?:Created|Status|Linked|Review|Passed)\.ariaSort \}\}"/g)||[]).length,5);
 assert.match(page,/class="pm-delivery-owner-filter"[\s\S]*<select aria-label="\{\{ delivery\.creatorFilterHint \}\}"[^>]*sc-camel-on-change="\{\{ delivery\.onCreatorFilter \}\}"/);
 assert.doesNotMatch(page,/创建人[\s\S]{0,300}(?:type="search"|role="combobox")/);
 ...
 assert.match(built,/\.forge-postman \.pm-delivery-table-head>:is\(span,label\)\{[^}]*white-space:nowrap/);
 assert.match(built,/\.forge-postman \.pm-delivery-sort-button\{[^}]*flex-direction:column[^}]*width:14px/);
 ...
 assert.deepEqual(Array.from(view.creatorOptions,option=>option.label),['全部','一万','allen','yokiguan']);
 assert.equal(view.creatorFilter,'all');
 view.onCreatorFilter({target:{value:'allen'}});
 view=c.renderVals().delivery;
 assert.equal(view.creatorFilter,'allen');
 assert(view.creatorFilterActive);
 assert(view.customers.flatMap(customer=>customer.sheets).every(sheet=>sheet.creator==='allen'));
 view.onCreatorFilter({target:{value:'all'}});
 view=c.renderVals().delivery;
 assert.equal(view.creatorFilter,'all');
 assert(!view.creatorFilterActive);
 view.sortLinked.toggle();
 view=c.renderVals().delivery;
 assert.equal(c.state.deliveryTableSortKey,'linked');
 assert.equal(c.state.deliveryTableSortDirection,'desc');
 assert.deepEqual(Array.from(view.customers.flatMap(customer=>customer.sheets),sheet=>sheet.linked),[200,152,120,96,84]);
 assert(view.sortLinked.active);
 assert(!view.sortCreated.active&&!view.sortStatus.active&&!view.sortReview.active&&!view.sortPassed.active);
 view.sortLinked.toggle();
 view=c.renderVals().delivery;
 assert.equal(c.state.deliveryTableSortDirection,'asc');
 assert.deepEqual(Array.from(view.customers.flatMap(customer=>customer.sheets),sheet=>sheet.linked),[84,96,120,152,200]);
 view.sortCreated.toggle();
 view=c.renderVals().delivery;
 assert.equal(c.state.deliveryTableSortKey,'created');
 assert.equal(c.state.deliveryTableSortDirection,'desc');
 assert.deepEqual(Array.from(view.customers.flatMap(customer=>customer.sheets),sheet=>sheet.createdDate),['2026/08/25','2026/08/22','2026/08/18','2026/08/12','2026/08/07']);
 assert(view.sortCreated.active&&!view.sortLinked.active);
