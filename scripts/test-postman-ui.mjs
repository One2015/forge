...
test('Forge v2 visual system uses a cool brand and covers every major product surface',()=>{
 assert.match(built,/--surface-canvas:#ffffff/);
...
test('overview delivery progress is a flat section without a sheet-count label',()=>{
 ...
 assert(group.rows.every(row=>/^\d+ 天后$/.test(row.deliveryDate)));
 assert.equal(group.sortAria,'ascending');
 assert.equal(group.sortLabel,'最近优先');
 assert.deepEqual(Array.from(group.rows,row=>row.deliveryDays),[7,10,13,16,19]);
 group.toggleSort();
 group=c.renderVals().over.groups[0];
 assert.equal(group.sortAria,'descending');
 assert.equal(group.sortLabel,'最晚优先');
