...
test('all main pages, details, filters and utility panels have stable round trips', () => {
  for (const url of [
    ...
    '/billing/suppliers', '/billing/overview?preset=custom&start=2026-09-01&end=2026-09-03&grain=day&time=day', '/billing/overview?metric=calls&sort=calls&direction=asc&page=2&pageSize=5&search=web&bin=2026-09-02T15', '/models?provider=test&status=attention&q=model', '/models?dimension=models&status=slow&selection=claude&route=yq-claude', '/models?period=24h', '/models?period=custom&from=2026-09-01T09%3A00&to=2026-09-02T18%3A30', '/models?source=live', '/outsourcing-suppliers', '/outsourcing-suppliers/stepfun?risk=high&cycle=7d', '/outsourcing-suppliers?tab=management', '/overview?panel=downloads',
...
test('aliases and legacy bookmarks resolve without replaying approval or mutations', () => {
  ...
  assert.equal(codec.write(codec.read('/outsourcing-suppliers?sheet=step300').patch), '/outsourcing-suppliers');
