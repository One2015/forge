...
test('status, search and owner filters compose below the read-only summary',()=>{const {q}=fixture();q().filters.find(f=>f.label==='运行失败').pick();assert.equal(q().total,2);q().onQuery({target:{value:'web3d-car'}});assert.equal(q().total,1);q().toggleMine();assert.equal(q().total,0);q().clear();assert.equal(q().total,9);assert(!q().hasFilters);q().filters.find(f=>f.label==='运行中').pick();assert.equal(q().total,2);q().filters.find(f=>f.label==='全部').pick();assert.equal(q().total,9);q().filters.find(f=>f.label==='运行完成').pick();assert.equal(q().total,5);});
...
test('summary cards are read-only and table filters remain interactive',()=>{const html=fs.readFileSync(new URL('./postman-ui/run-records.html',import.meta.url),'utf8');assert.doesNotMatch(html,/runs.groups|分组：|失败 Item|需立即处理|rr-kpi|k\.selected|k\.pick/);assert.match(html,/data-actionable="false"/);assert.equal((html.match(/role="columnheader"/g)||[]).length,7);assert.match(html,/rr-context/);assert.match(html,/sc-camel-on-input/);assert.match(html,/aria-pressed="{{ f.selected }}"/);assert(html.indexOf('只看我的')>html.indexOf('rr-status-tabs'));assert(html.indexOf('只看我的')<html.indexOf('aria-label="搜索运行、流程或数据集"'));});
test('desktop search occupies the right edge after status and owner filters',()=>{const css=fs.readFileSync(new URL('../public/postman-ui/global-responsive.css',import.meta.url),'utf8');assert.match(css,/@media\(min-width:761px\)\{[\s\S]*?\.forge-postman \.rr-filter-controls\{flex:0 1 300px;flex-wrap:nowrap;margin-left:auto\}/);});
