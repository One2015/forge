...
test('status, search and owner filters compose below the read-only summary',()=>{const {q}=fixture();q().setFilter({target:{value:'运行失败'}});assert.equal(q().total,2);q().onQuery({target:{value:'web3d-car'}});assert.equal(q().total,1);q().toggleMine();assert.equal(q().total,0);q().clear();assert.equal(q().total,9);assert(!q().hasFilters);q().filters.find(f=>f.label==='运行中').pick();assert.equal(q().total,2);q().filters.find(f=>f.label==='全部').pick();assert.equal(q().total,9);q().filters.find(f=>f.label==='运行完成').pick();assert.equal(q().total,5);});
...
test('every run item row, including failures, opens lifecycle detail',()=>{
 ...
 assert(failed);assert(review);assert(items.every(item=>item.rowCursor==='pointer'&&item.rowTitle==='查看条目详情'&&typeof item.open==='function'&&typeof item.keyOpen==='function'));
 ...
 const detail=c.renderVals();assert(detail.explorer.hasRunOverview);assert.equal(detail.explorer.runState,'失败');assert.equal(detail.explorer.runProgress,'11 / 18');assert.equal(detail.explorer.runNode,'runtime');assert.equal(detail.explorer.runCostTime,'$35.80 · 16 分 15 秒');assert.equal(detail.life.backLabel,'返回运行详情');
 ...
 assert.match(built,/role="link" tabindex="0" aria-label="查看条目 \{\{ it\.id \}\} 详情"/);
});
...
test('summary cards are read-only and table filters remain interactive',()=>{const html=fs.readFileSync(new URL('./postman-ui/run-records.html',import.meta.url),'utf8');assert.doesNotMatch(html,/runs.groups|分组：|失败 Item|需立即处理|rr-kpi|k\.selected|k\.pick/);assert.match(html,/data-actionable="false"/);assert.equal((html.match(/role="columnheader"/g)||[]).length,7);assert.match(html,/rr-context/);assert.match(html,/sc-camel-on-input/);assert.match(html,/sc-camel-on-change="{{ runs.setFilter }}"/);assert.doesNotMatch(html,/rr-status-tabs/);assert(html.indexOf("新建运行")>html.indexOf('aria-label="搜索运行、流程或数据集"'));assert(html.indexOf('只看我的')>html.indexOf('rr-status-filter'));assert(html.indexOf('只看我的')<html.indexOf('aria-label="搜索运行、流程或数据集"'));});
test('desktop search occupies the right edge after status and owner filters',()=>{const css=fs.readFileSync(new URL('../public/postman-ui/global-responsive.css',import.meta.url),'utf8');assert.match(css,/@media\(min-width:761px\)\{[\s\S]*?\.forge-postman \.rr-filter-controls\{flex:0 1 460px;flex-wrap:nowrap;margin-left:auto\}/);});
test('execution duration prefers real timings and explicitly marks legacy snapshots',()=>{
 const {api,q}=fixture();assert(q().rows.every(r=>r.durationLabel!=='—'));assert(q().rows.every(r=>r.durationSynthetic));assert(q().rows.every(r=>/[秒分时]/.test(r.durationLabel)&&!/\d\s*[hms]\b/.test(r.durationLabel)));assert(q().rows.every(r=>r.durationHint.startsWith('示例执行时长')));
...
test('production run copy is Chinese while technical identifiers stay intact',()=>{
 const {c,q,codec}=fixture();
 assert(q().rows.every(row=>!/(Mock|workspace|Run ID)/.test(row.durationHint+' '+row.strategy+' '+row.copyLabel)));
 assert(q().rows.some(row=>row.strategy==='复用专家工作区'));
 Object.assign(c.state,codec.read('/production/runs/20260825-093412-a4f7c1').patch);
 const values=c.renderVals();
 assert(values.subNav.some(tab=>tab.label==='流程'));
 assert.equal(values.run.title,'科学机理组');
 assert.match(values.run.id,/ · 8 条$/);
 assert(values.run.items.every(item=>!/[A-Za-z]{4,}\s+[A-Za-z]{4,}/.test(item.domain)));
 assert.equal(values.profile.roleName,'成员');
 assert.match(built,/forge-sidebar-profile-label[^>]*>\{\{ profile\.roleName \}\}/);
 const html=fs.readFileSync(new URL('./postman-ui/run-records.html',import.meta.url),'utf8');
 assert.doesNotMatch(html,/>Pipeline<|Item 状态|Run ID:/);
});
