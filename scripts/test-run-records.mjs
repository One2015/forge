...
test('every execution state has one visible operation and no overflow menu',()=>{
 ...
 const css=fs.readFileSync(new URL('../public/postman-ui/run-records.css',import.meta.url),'utf8');assert.match(fs.readFileSync(new URL('../public/postman-ui/forge-system.css',import.meta.url),'utf8'),/\[data-kind=review\]\{--rr-action-ink:var\(--accent-primary\)/);assert.match(css,/\.rr-bar \[data-state=review\],\.rr-bar \[data-state=passed\]\{background:var\(--status-success\)\}/);
});

test('operation filter matches button actions and composes with other filters',()=>{
 const {q}=fixture();
 for(const [key,label,count] of [['review','人工审核',6],['cause','查看原因',2],['progress','查看进度',1]]){
  q().actionFilter.options.find(o=>o.value===key).pick();
  assert.equal(q().total,count);assert(q().rows.every(r=>r.actionLabel===label));assert(q().hasFilters);
 }
 q().onQuery({target:{value:'no-matching-run'}});assert(q().empty);
 q().clear();assert.equal(q().total,9);assert.equal(q().filterCount,0);
 q().actionFilter.options.find(o=>o.value==='cause').pick();
