...
test('no dataset group headings and no colored summary figures',()=>{const html=fs.readFileSync(new URL('./postman-ui/run-records.html',import.meta.url),'utf8');assert.doesNotMatch(html,/runs.groups|分组：|失败 Item|需立即处理/);assert.equal((html.match(/role="columnheader"/g)||[]).length,7);assert.match(html,/rr-context/);assert.match(html,/sc-camel-on-input/);assert.match(html,/aria-pressed="{{ k.selected }}"/);assert(html.indexOf('只看我的')>html.indexOf('rr-status-tabs'));assert(html.indexOf('只看我的')<html.indexOf('aria-label="搜索运行、Pipeline 或数据集"'));});
