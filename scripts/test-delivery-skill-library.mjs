...
function component(props = {}) {
  ...
  c.openDeliveryEditor(); c.patchDeliveryEditor({ tab: 'skills', name: 'Skill 测试交付单', customer: '测试客户', target: '1' });
...
test('same-content uploads reuse catalog entries without selection; selected command collisions get a unique alias', async () => {
  ...
  c.deliveryEditorValues().library.rows.find(row => !row.selected).toggle();
...
test('selection cap does not prevent independent creation; selected rows remain removable', async () => {
  ...
  c.deliveryEditorValues().workspace.publishUploads();
  assert.equal(c.platformSkillCatalog().length, 14);
...
test('focused generator is idempotent and searchable rows expose keyboard and selection semantics', () => {
  ...
  assert(template.includes('<label class="forge-delivery-library-option" data-selected="{{ option.selected }}"'));
  assert(template.includes('<input type="checkbox" aria-label="{{ option.actionLabel }}"'));
  assert(template.includes('checked="{{ option.selected }}"'));
  assert(template.includes('sc-camel-on-change="{{ option.toggle }}"'));
  ...
  assert(template.includes('.forge-delivery-library-option:has(input:focus-visible)'));
  assert(!template.includes('class="forge-delivery-library-action"'));
  assert(template.includes('.forge-delivery-editor.forge-feedback .forge-delivery-skill-search input[type="search"]{padding-left:34px;padding-right:36px}'));
  assert(template.includes('.forge-delivery-library-option>input[type="checkbox"]{display:block;flex:none;width:16px;height:16px;min-height:0;'));
});
