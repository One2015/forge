...
本变体的新 Postman 规则优先于相冲突的 Geist 视觉要求；原版保留作比较。业务含义与已有方法继续沿用：目标、已关联、已跑完、待审核、可交付不能因重排而改义；费用分析继续遵守 `PRODUCT.md` 的自然日与示例账单约束。
...
- **导航与标签**：模块选中用纯白底、橙色图标/侧边条和 600 字重；生产、费用与编辑局部标签统一为纯白选中底和深灰细线。数据集归属“生产”，创建数据单归属“交付”，选中态跟随当前页面归属。
...
- **Do** 保留原版、明确示例数据与未接入服务，维持原有路由归属校验。
