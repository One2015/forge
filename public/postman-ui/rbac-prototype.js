...
返回通过数、异常数以及逐条问题明细，不自动覆盖原始数据。`,
  ...
  function routeHost() {
    try {
      if (window.frameElement?.classList?.contains('forge-frame') && window.parent.location.origin === window.location.origin) return window.parent;
    } catch (_) {}
    return window;
  }
  function routeUrl() {
  ...
  function isLegacyProfileRoute() {
    ...
    if (host === window && /\/forge-(?:postman|rbac)\.html$/.test(url.pathname)) {
      ...
      return new URL(nested, 'https://forge.invalid').searchParams.get('panel') === 'profile';
    }
  ...
  function mount() {
    ...
    root.setAttribute('aria-labelledby', 'forge-rbac-page-title');
    root.setAttribute('tabindex', '-1');
    (document.querySelector('.forge-main') || document.body).appendChild(root);
    ...
    routeHost().addEventListener('popstate', syncWorkspaceToRoute);
    applyFeatureAccess();
    render();
  ...
  function closeWorkspace() {
    ...
    root.dataset.open = 'false';
    delete document.body.dataset.rbacPage;
    const profile = document.querySelector('.forge-sidebar-profile');
    if (profile) {
  ...
  function renderDock() {
    ...
    dock.querySelector('[data-dock="project"]')?.addEventListener('change', event => {
      ...
      if (event.target.value === 'external-expert' && !isAdmin()) openWorkspace('profile', { syncRoute: true });
